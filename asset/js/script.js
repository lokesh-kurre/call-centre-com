...
$document.ready(function () {
   ...
   */
  if (isDesktop && $html.hasClass("wow-animation") && !isNoviBuilder && plugins.wow.length) {
    new WOW().init();
  }
    ...
    for (i = 0; i < plugins.rdMailForm.length; i++) {
      ...
      $form.attr('novalidate', 'novalidate').ajaxForm({
        ...
        success: function (result) {
          ...
          if (result === "MF000") {
            if (output.hasClass("snackbars")) {
              output.html('<p>' + msg[result] + '</p>');
            } else {
              output.addClass("active success");
            }
          } else {
            if (output.hasClass("snackbars")) {
              output.html(' <p class="snackbars-left">' + msg[result] + '</p>');
            } else {
  ...
  if (plugins.pageLoader.length && !isNoviBuilder) {
    setTimeout(function () {
      plugins.pageLoader.fadeOut(200, function () {
        $(this).remove();
      });
